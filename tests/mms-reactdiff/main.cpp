...
// u1=0.5*sin(pi x)sin(pi y). Returns 0 (pass) / 1 (fail). Runs on CPU or GPU (compile-time
// backend macro) and single- or multi-rank (owned elements verified; ghosts excluded).
#include <mpi.h>
#include <cmath>
#include <cstdio>
#include <vector>
...
int main(int argc,char**argv){
  ...
  // LOCAL is needed for the backend's distributed HDG trace halo exchange (hdgMatVec / MPI
  // assembly), which defaults to MPI_COMM_NULL; for a single model it is MPI_COMM_WORLD.
  EXASIM_COMM_WORLD=MPI_COMM_WORLD; EXASIM_COMM_LOCAL=MPI_COMM_WORLD;
  int rc=1;
  ...
  { MPI_Comm shm; int sr=0; MPI_Comm_split_type(MPI_COMM_WORLD,MPI_COMM_TYPE_SHARED,0,MPI_INFO_NULL,&shm);
    MPI_Comm_rank(shm,&sr); MPI_Comm_free(&shm);
    int dc=0; cudaGetDeviceCount(&dc); if(dc>0) cudaSetDevice(sr%dc); }
#endif
  { const char* filein = argc>1?argv[1]:"datain/";
    CSolution<PdeModel> model(filein,"dataout/out","",(Int)size,(Int)rank,0,(Int)rank,backend,100);
    ...
    auto& d=model.disc;
    const Int npe=d.common.grid.npe, ncx=d.common.components.ncx, nc=d.common.components.nc;
    const Int ne=d.common.meshsizes.ne, ne1=d.common.meshsizes.ne1;
    ...
    // Verify OWNED elements [0,ne1) only: ghost/halo elements [ne1,ne) are neighbor copies that
    // do not hold this rank's solution, so they legitimately mismatch the exact field.
    double emax=0;
    for(Int e=0;e<ne1;++e) for(Int p=0;p<npe;++p){
    ...
    // Reduce across ranks: global worst error and worst (min) convergence reason.
    double gemax=emax; int greason=reason;
    MPI_Allreduce(&emax,&gemax,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);
    MPI_Allreduce(&reason,&greason,1,MPI_INT,MPI_MIN,MPI_COMM_WORLD);
    const double TOL=2e-2;
    if(rank==0)
      printf("[mms] np=%d SNESConvergedReason=%d  max|u-u_exact|=%.3e  (tol %.0e)  %s\n",
             size, greason, gemax, TOL, (greason>0 && gemax<TOL)?"PASS":"FAIL");
    rc = (greason>0 && gemax<TOL)?0:1;
  }

function [nlg, xg] = surfacenormal(mesh, master, wid)
%SURFACENORMAL Compute outward normals on selected boundary faces.
%
%   nlg = surfacenormal(mesh, master, wid)
%   [nlg, xg] = surfacenormal(mesh, master, wid)
%
% wid may contain one or more boundary markers. If mesh.boundarycondition
% exists, wid may also contain boundary-condition values; these are mapped to
% the corresponding boundary marker ids. The outputs have size ngf x nd x nf,
% where ngf is the number of face quadrature points, nd is the physical
% dimension, and nf is the total number of selected boundary faces. xg contains
% the physical quadrature-point coordinates on the selected boundary faces.

validateInputs(mesh, master, wid);
...
function validateInputs(mesh, master, wid)
if ~isstruct(mesh)
...
nd = size(mesh.dgnodes, 2);
if nd ~= 2 && nd ~= 3
    error('surfacenormal supports only 2D and 3D meshes.');
end

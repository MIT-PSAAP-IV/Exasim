...
for i = 1:ns
  ...
  h1 = sum(c1.*fT); %  nondimensional Gibbs free energy 
  h2 = sum(c2.*fT); %  nondimensional Gibbs free energy 
  h3 = sum(c3.*fT); %  nondimensional Gibbs free energy   
  Gformation(i) = fsw(1)*h1 + fsw(2)*h2 + fsw(3)*h3 - pressureTerm;
end

% natural log of the forward reaction rate 
lnkf_r = sym(zeros(1,nr));
for ir = 1:nr  
  lnkf_r(ir) = log(A_r(ir)) + beta_r(ir) * fT(8) - theta_r(ir)*fT(6);
end

% The stoichiometric coefficients of species s in the reactants and products of reaction r
nu_bf = zeros(ns, nr);
for ir = 1:nr
  for k = 1:ns
...
% natural log of the backward reaction rate 
...
kf_r = exp(lnkf_r); % forward reaction rate 
kb_r = exp(lnkb_r); % backward reaction rate 
...
% kinetic source terms for each species

function [rho_species, rhoE_new, info] = init_reacting5_from_ideal(rho, rhou, p, T)
...
%   info        : struct with fields x (mole frac), Y (mass frac), lnKp, iters, converged, p_from_eos
    ...
    rho_species = rho .* Y;       % [rho_N rho_O rho_NO rho_N2 rho_O2]
    ...
    for i = 1:5
        ...
        h_molar(i) = H * Ru * T;                           % J/mol
        Ha(i) = H;
    end
    Rspec  = Ru ./ W;             % J/(kg*K), order [N O NO N2 O2]
    h_mass = h_molar ./ W;        % J/kg
    e_mass = h_mass - Rspec * T;  % J/kg    
    
    e_mix   = sum(Y .* e_mass); % e = RU * T * (sum(Y./W .*  H) - sum(Y./W))
    % if e_mix < 0
    %   disp("e_mix is negative");
    %   disp(e_mix);
    %   disp(sum(Y./W .*  Ha) - sum(Y./W));
    %   disp(Ha);
    %   disp(Y);
    %   disp(Y./W);
    %   disp(rho_species);
    ...
    info.p_from_eos = rho * (sum(Y .* Rspec)) * T; % implied pressure from mixture EOS
end

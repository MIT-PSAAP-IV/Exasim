function ub = ubound(u, q, w, v, x, t, mu, eta, uh, n, tau, uinf, param, ib)
%UBOUND LDG exterior states for the five-species reacting-air model.
%
...
%   8 partial-catalysis wall, 9 stoichiometric partial-catalysis wall.
    ...
    if numel(u) ~= ncu
        error('ubound:InvalidUSize', 'u must have length %d, but got %d.', ncu, numel(u));
    end
    if numel(w) < 1
        error('ubound:InvalidWSize', 'w must contain at least one entry.');
    end
    if numel(mu) < 12
        error('ubound:InvalidMuSize', 'mu must contain at least 12 entries.');
    end
    if numel(eta) < ncu + 2*ns
        error('ubound:InvalidEtaSize', ...
    ...
    % 2) Outflow and 5) zero-gradient: extrapolate the interior state.
    ub_out = u;
    ub_grad = u;

    % 3) Isothermal no-slip wall: preserve species density, zero velocity,
    % recompute total energy from T_wall.
    ...
    % 4) Slip wall: remove the normal momentum component only.
    m = u(irhou);
    mn = m.' * n;
    ub_slip = u;
    ...
    % 6) Noncatalytic wall: preserve flow mass fractions at wall temperature.
    ...
    % 8-9) Catalytic flux walls use the isothermal wall trace; fbound replaces
    % the species normal flux with the catalytic wall flux.
    ub_cat_gam = ub_iso;
    ...
    elseif ib >= 1 && ib <= size(ub_all, 2)
        ...
        error('ubound:InvalidBoundaryIndex', ...

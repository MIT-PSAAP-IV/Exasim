void cpuInitu(dstype* f, const dstype* xdg, const dstype* uinf, const dstype* param, const int modelnumber, const int ng, const int ncx, const int nce, const int npe, const int ne)
{
	for (int i = 0; i <ng; i++) {
		int j = i%npe;
		int k = i/npe;
		f[j+npe*0+npe*nce*k] = 1.0;
		f[j+npe*1+npe*nce*k] = 1.0;
		f[j+npe*2+npe*nce*k] = 1.0;
		f[j+npe*3+npe*nce*k] = 1.0;
		f[j+npe*4+npe*nce*k] = 1.0;
		f[j+npe*5+npe*nce*k] = 1.0;
		f[j+npe*6+npe*nce*k] = 1.0;
		f[j+npe*7+npe*nce*k] = 1.0;
		f[j+npe*8+npe*nce*k] = 1.0;
	}
}


function fb = fbouhdgnd(u, q, w, v, x, t, mu, eta, uhat, n, tau)
...
%   - Supports nd = 2 or 3.
    ...
    if nd == 1
      [~, f_species_iso] = fluxcart1d(uf, q, w_wall, v, x, t, mu, eta);
      %[~, f_species_iso] = fluxaxial1d(uf, q, w_wall, v, x, t, mu, eta);
    elseif nd == 2
      [~, f_species_iso] = fluxcart2d(uf, q, w_wall, v, x, t, mu, eta);
      %[~, ~, ~, f_species_iso] = fluxaxial2d(uf, q, w_wall, v, x, t, mu, eta);
    elseif nd == 3
      [~, f_species_iso] = fluxcart3d(uf, q, w_wall, v, x, t, mu, eta);  
...
% function fb = fbouhdgnd(u, q, w, v, x, t, mu, eta, uhat, n, tau)
% %FBOUHDGND HDG boundary state differences / flux-like boundary terms.
% %
% % State ordering (for ns = 5 species):
% %   u = [rho_1, ..., rho_ns, rho*u_1, ..., rho*u_nd, rhoE]^T
% %
% % Inputs:
% %   u     : interior state, size ncu x 1
% %   q     : gradient-like variable, size (ncu*nd) x 1 or ncu x nd
% %   x     : spatial coordinate, size nd x 1
% %   mu   : parameter vector
% %           mu(1)  = rho_scale
% %           mu(3)  = rhoe_scale
% %           mu(12) = T_wall
% %   uhat  : trace state, size ncu x 1
% %   n     : outward unit normal, size nd x 1
% %   tau   : stabilization parameter (scalar or compatible size)
% %   ubou  : prescribed boundary state, size ncu x 1
% %
% % Output:
% %   fb    : ncu x 5 matrix whose columns correspond to:
% %           [inflow, outflow, isothermal wall, slip wall, zero-gradient]
% %
% % Notes:
% %   This implementation assumes ns = 5 reacting-air species.
% %   It supports nd = 2 or 3.
% %
% %   The returned quantities are constructed in the same "difference-to-uhat"
% %   spirit as the original code, with tau scaling applied consistently.
% 
%     % -------------------------------------------------
%     % Basic dimensions
%     % -------------------------------------------------
...
%     else
%         q = q(:);
%         if numel(q) ~= ncu_expected * nd
%             error('fbouhdgnd:InvalidQSize', ...
%                 'q must have size ncu*nd = %d, but got %d entries.', ...
%                 ncu_expected * nd, numel(q));
%         end
%         qmat = reshape(q, [ncu_expected, nd]);
%     end
% 
%     % -------------------------------------------------
%     % Input checks
%     % -------------------------------------------------
%     if numel(u) ~= ncu_expected
%         error('fbouhdgnd:InvalidUSize', ...
%             'u must have length %d, but got %d.', ncu_expected, numel(u));
%     end
% 
%     if numel(uhat) ~= ncu_expected
%         error('fbouhdgnd:InvalidUhatSize', ...
%             'uhat must have length %d, but got %d.', ncu_expected, numel(uhat));
%     end
% 
%     if numel(ub) ~= ncu_expected
%         error('fbouhdgnd:InvalidUbouSize', ...
%             'ubou must have length %d, but got %d.', ncu_expected, numel(ub));
%     end
% 
%     if numel(n) ~= nd
%         error('fbouhdgnd:InvalidNormalSize', ...
...
%     % -------------------------------------------------
%     % 3) Isothermal no-slip wall
%     %    - keep species from interior state
%     %    - zero momentum
%     %    - recompute rhoE from species densities and wall temperature
%     % -------------------------------------------------
%     rho_i_wall      = u(irho);
%     rho_i_wall_dim  = rho_i_wall * rho_scale;
% 
%     % Wall velocity: zero vector of size nd
%     v_wall = zeros(nd, 1);
% 
%     [rhoE_dim, ~, ~] = energyFromSpecies(rho_i_wall_dim, T_wall, v_wall);
% 
...
%     % -------------------------------------------------
%     % 4) Slip wall
%     %    Remove only the normal momentum component:
%     %       m_t = m - (m.n) n
%     % -------------------------------------------------
%     m = u(irhou);          % momentum vector
%     mn = m(1)*n(1);        % normal momentum component
%     for d = 2:nd
%       mn = mn + m(d)*n(d);
%     end
% 
...
%     % -------------------------------------------------
%     % 5) Zero-gradient / Neumann-like condition
%     %    qmat is ncu x nd, so q*n is the normal derivative-like contribution
%     % -------------------------------------------------    
%     qn = qmat(:,1)*n(1);
%     for d = 2:nd
%       qn = qn + qmat(:,d)*n(d);
...
% 
%     %%% Noncatalytic wall
%     % Set wall mass fractions equal to the flow mass fractions
%     % (discrete analogue of dY/dn = 0), set wall temperature = T_wall,
%     % and set wall pressure equal to the local flow pressure.
% 
%     % Flow thermodynamic state
%     T_flow     = w(1) * T_scale;
%     rho_i_flow = u(1:ns) * rho_scale;
% 
%     % Flow pressure
%     p_flow = pressure(T_flow, rho_i_flow, Mw);
% 
%     % Flow mass fractions (computed directly)
%     rho_flow  = sum(rho_i_flow);
%     Y_i_flow  = rho_i_flow / rho_flow;
% 
%     % Wall mixture density from (T_wall, p_flow, Y_i_flow)
%     rho_noncat   = densityFromMassFractions(T_wall, p_flow, Y_i_flow, Mw);
%     rho_i_noncat = rho_noncat * Y_i_flow;
% 
%     % Compute wall total energy density
%     [rhoE_dim_noncat, ~, ~] = energyFromSpecies(rho_i_noncat, T_wall, v_wall);
% 
%     % Build nondimensional wall state
%     u_noncat = 0 * uhat;
%     u_noncat(1:ns) = rho_i_noncat / rho_scale;
%     u_noncat((ns+1):(ns+nd)) = 0;
%     u_noncat(ns+nd+1)   = rhoE_dim_noncat / rhoe_scale;
% 
%     % Boundary residual
%     f_noncat = tau*(u_noncat - uhat);
% 
%     %%% Supercatalytic wall 
%     % specify Y_i to Y_eq
%     % Step 1: get pressure from flow computed above
%     % Step 2: specify mass fractions
%     Y_i_cat = eta((ns+1):2*ns);
% 
%     % Wall mixture density from (T_wall, p_flow, Y_i_flow)
%     rho_cat   = densityFromMassFractions(T_wall, p_flow, Y_i_cat, Mw);
%     rho_i_cat = rho_cat * Y_i_cat;
...
%     % Build nondimensional wall state
%     u_cat = 0 * uhat;
%     u_cat(1:ns) = rho_i_cat / rho_scale;
%     u_cat((ns+1):(ns+nd)) = 0;
%     u_cat(ns+nd+1)   = rhoE_dim_cat / rhoe_scale;
% 
%     % Boundary residual
%     f_cat = tau*(u_cat - uhat);
% 
...
%     % J_i · n = w_cat
% 
%     gam_i = eta(2*ns+1:3*ns);
% 
%     w_wall = T_wall / T_scale;
% 
%     uf = u;
%     uf(ns+1:ns+nd) = 0;
%     uf(ns+nd+1) = rhoE_wall;
% 
%     f_species_iso = flux_visc_species(uf, q, w_wall, v, x, t, mu, eta);
% 
%     % Normal viscous species flux
%     fn_species_iso = 0 * f_species_iso(:,1);
%     for d = 1:nd
%         fn_species_iso = fn_species_iso + f_species_iso(:,d) * n(d);
%     end
% 
%     % Catalytic wall mass flux
%     J_cat = 0 * fn_species_iso;
%     for is = 1:ns
%         J_cat(is) = gam_i(is) * sqrt(T_wall / (2*pi)) ...
%                   * sqrt(RU / Mw(is)) * u(is) * rho_scale;
%     end
% 
%     % Add catalytic contribution in nondimensional form
%     fn_species_iso(1:ns) = fn_species_iso(1:ns) + J_cat / (rho_scale * u_scale);
% 
%     % Final species boundary flux
%     f_cat_gam = f_iso;
%     f_cat_gam(1:ns) = fn_species_iso + tau * (u(1:ns) - uhat(1:ns));
% 
%     % -------------------------------------------------
%     % Collect all boundary conditions
%     % -------------------------------------------------
%     fb = [f_in, f_out, f_iso, f_slip, f_grad, f_noncat, f_cat, f_cat_gam];
% end
% 

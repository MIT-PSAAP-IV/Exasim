...
%PDEMODEL_CART Calorically perfect ideal-gas compressible Navier--Stokes.
%
...
fb = cnsig_fbou_from_states(ub, q, v, x, mu, n, tau, uhat, qn, false);
end

function fb = fbouhdg(u, q, w, v, x, t, mu, eta, uhat, n, tau) %#ok<INUSD>
[ub, qn] = cnsig_boundary_states(u, q, x, mu, eta, n);
fb = cnsig_fbouhdg_from_states(ub, q, v, x, mu, n, tau, uhat, qn, false);
end
...
% Stationary adiabatic no-slip wall: no mass flux and zero total-energy
% flux. Momentum retains the wall pressure/viscous contribution from the
% no-slip exterior state plus velocity penalization.
fb(:,4) = cnsig_normal_flux(ub(:,4), q, v, x, mu, n, axial) + tau.*(ub(:,4) - uhat);
fb(1,4) = 0;
fb(ncu,4) = 0;

% Symmetry in conservative variables: (rho*u).n = 0, grad(rho).n = 0,
% grad(rhoE).n = 0, and grad(tangential momentum).n = 0.
fb(:,5) = cnsig_symmetry_residual(ub(:,6), q, x, mu, n, tau, uhat, axial);

% Zero-normal-gradient condition: q is Exasim's first-order variable.
...
% Adiabatic no-slip wall: density trace continuity, zero trace momentum,
% and an energy normal-flux equation. This is the same HDG wall structure
% used by existing ideal-gas/sCO2 Exasim Navier--Stokes examples.
fad = 0*uhat;
fad(1) = ub(1,4) - uhat(1);
...
function [gvel, gT] = cnsig_grad_primitives_axial(u, gU, x, mu) %#ok<INUSD>
[gvel, gT] = cnsig_grad_primitives(u, gU, mu);
end

function f = cnsig_flux_axial(u, q, v, x, mu)
% Unused in the Cartesian model; present only because the boundary helper is
% shared with pdemodel_axial.m and keeps all local function references
% resolvable during parsing/code generation.
f = cnsig_flux_cart(u, q, v, x, mu);
end

from .hdggencodebou2 import hdggencodebou2

def hdggencodeface2(filename, f, xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time, foldername):
    strkk = ""
    nbc = f.shape[1]
    for k in range(nbc):
        str1 = hdggencodebou2(filename + str(k + 1), f[:, k], xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time)
        strkk += str1
    ...
    for k in range(nbc):
        ...
        tmp += "\t\t" + cpufile + str(k + 1) + "(f, xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time, modelnumber, ng, nc, ncu, nd, ncx, nco, ncw);\n"
    tmp += "}\n\n"

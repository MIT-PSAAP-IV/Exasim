...
//     printf("MPI rank %d using GPU %d and CPU %d on host %s\n", mpirank, device, cpu, hostname );
  ...
  {        
    ...
    for (int i=0; i<nummodels; i++) {        
        if (mpiprocs0==0) {
          pdemodel[i] = new CSolution<Model>(filein[i], fileout[i], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend);                 
        }
        else if (mpiprocs0 > 0) {
          if (mpirank < mpiprocs0) { 
            pdemodel[i] = new CSolution<Model>(filein[0], fileout[0], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend);       
          }
          else {
            fileoffset = mpiprocs0;
            pdemodel[i] = new CSolution<Model>(filein[1], fileout[1], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend);       
          }
        }

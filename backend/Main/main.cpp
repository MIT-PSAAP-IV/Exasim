...
const char* SelectExasimDriverProviderName(const int builtinmodelID)
{
    if (builtinmodelID > 0) return "BuiltIn";
...
void PrintModelProvider(const int modelnumber, const int builtinmodelID)
{
    ...
    if (rank == 0) {
        std::cout << "Model " << modelnumber
                  << ": provider = " << SelectExasimDriverProviderName(builtinmodelID)
...
int ConfigureModelDefinitions(ExasimSolver& solver)
{
    for (int i = 0; i < solver.NumModelDefinitions(); i++) {
        ...
        const int err = solver.SetModelDefinition(i, builtinmodelID, SelectExasimDriverABI(builtinmodelID));
        if (err) return err;
...
int ConfigureModelDefinitions(ExasimSolver& solver, const std::vector<int>& vecBuiltinModelID)
{
    ...
    for (int i = 0; i < numModelDefinitions; i++) {
        ...
        if (builtinmodelID <= 0) builtinmodelID = solver.BuiltinModelID(i);
...
// 
// #include "../Model/FrontendGenerated/frontendprovider.cpp" // discretization class
// 

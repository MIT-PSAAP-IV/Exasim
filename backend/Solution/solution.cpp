...
//     disc.compMassInverse(backend);
    ...
    writer.SaveNodesOnBoundary(backend);     
...
{    
    ...
    {            
        ...
        writer.SaveSolutions(backend);
        writer.SaveQoI(backend);
        if (vis.savemode > 0) writer.SaveParaview(backend); 
        writer.SaveSolutionsOnBoundary(backend); 
        if (disc.common.components.nce>0)
            writer.SaveOutputCG(backend);    
...
void CSolution::SteadyProblem_PTC(ofstream &out, Int backend) {
    ...
    {            
        ...
        for (int j=0; j<disc.common.timeparams.tstages; j++) {     
            ...
            writer.evalMonitor(disc.tmp.tempg,  solv.sys.udgprev, solv.sys.wprev, disc.common.components.ncu, backend);

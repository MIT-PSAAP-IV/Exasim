...
static void PrintErrorAndExit(const std::string& errmsg, const char *file, int line ) 
{    
...
//     std::filesystem::path full = base / str2;     // joins with the right separator
//     return full.string();            // or .native() on Windows if you prefer backslashes
// }
...
template <typename T> 
void readarray(std::ifstream &in, std::vector<T> &a, int N)
{    
    if (N>0) {        
        a.resize(N);
...
void readiarrayfromdouble(std::ifstream &in, std::vector<int> &a, int N) {
    if (N > 0) {
        a.resize(N);
        double read;
        for (int i = 0; i < N; i++) {
...
void readfile(std::string filename, std::vector<int> &ndims, std::vector<int> &data, int m) 
{
...
void readfile(std::string filename, std::vector<int> &ndims, std::vector<double> &data, int m) 
{
    std::ifstream in(filename, std::ios::in | std::ios::binary);
    if (!in) error("Unable to open file " + filename);

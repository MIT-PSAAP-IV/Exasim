...
//         }
        ...
        // The common periodic case maps each target vertex once and the
        // source/target vertex sets are disjoint. In that case, a single
        // remap pass over t is equivalent to the legacy ordered scan but avoids
        // scanning all elements once per matched vertex. Fall back when the
        // mapping could depend on replacement order.
...
void interface_elements(std::vector<int>& inte, std::vector<int>& intl, std::vector<int>& f, int nfe, int ne, int coupledinterface)
{
    int n=0;
    for (int i=0; i<ne; i++)
      for (int j=0; j<nfe; j++)

...
inline void writesol(const PDE& pde, const Mesh& mesh, const Master& master, const std::vector<int>& elempart, const std::string& filename) 
{
...
inline void writeVectorAsDoubles(std::ofstream& out, const std::vector<int>& a) {
...
static inline std::filesystem::path resolve_input_path(const std::string& base, const std::string& filename)
{
...
static inline void write_materialdatabase_values_as_binary(const std::filesystem::path& source,
...
static inline void stageMaterialDatabase(const PDE& pde)
{
        ...
        //       }
            ...
            for (int n=0; n<pde.mpiprocs; n++) {
                ...
                // else {
                    ...
                    bf.resize(mesh.nfe * ne); 
                    int* fi = (int*)malloc(mesh.nfe * ne * sizeof(int));      
                    select_columns(fi, mesh.f.data(), dmd[n].elempart.data(), mesh.nfe, ne);       
                    apply_bcm(bf.data(), fi, mesh.boundaryConditions.data(), mesh.nfe*ne, mesh.nbcm);                              
                    CPUFREE(fi);
                //}
                
                writesol(pde, mesh, master, dmd[n].elempart, make_path(pde.datainpath, "sol" + std::to_string(n+1) + ".bin"));    
                writemesh(pde, mesh, master, dmd[n], bf, make_path(pde.datainpath, "mesh" + std::to_string(n+1) + ".bin"));
            }               
        } else {
            ...
            writesol(pde, mesh, master, dmd.elempart, make_path(pde.datainpath , "sol.bin"));    
            writemesh(pde, mesh, master, dmd, bf, make_path(pde.datainpath, "mesh.bin"));      
        }    
    }

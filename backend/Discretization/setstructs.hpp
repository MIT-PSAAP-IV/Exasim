...
inline void settempstruct(tempstructT<T,I> &tmp, appstructT<T,I> &app, masterstructT<T,I> &master, meshstructT<T,I> &mesh, Int backend)
{
    ...
    if (nmaterialdb_prop > 0) {
      ...
      Int ngaMax = std::max(npe*neb, std::max(nge*neb, ngf*nfb*nfe));
      ...
      n3 = std::max(n3, materialValueScratch);
      n3 = std::max(n3, materialValueDaeScratch);
      n3 = std::max(n3, materialDerivativeScratch);
...
//       }
      ...
      M = 2*(common.meshsizes.nfe-1)*common.meshsizes.nf;

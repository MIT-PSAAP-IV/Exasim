...
Re = 6.84e6;%9.84e5;                    % Reynolds number
Pr = 0.71;                      % Prandtl number    
Minf = 5.43;%21.38;                   % Mach number
Tref  = 1274;%260.6;
Twall = 293.2;
pinf = 1/(gam*Minf^2);
Tinf = pinf/(gam-1);
...
disp("Iter 2")
mesh.vdg(:,1,:) = 0.008*tanh(nm*dist.*ld);
mesh.udg = sol;
...
disp("Iter 3")
mesh.vdg(:,1,:) = 0.0005*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 4")
mesh.vdg(:,1,:) = 0.0003*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 5")
mesh.vdg(:,1,:) = 0.0002*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 6")
mesh.vdg(:,1,:) = 0.00014*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 7")
mesh.vdg(:,1,:) = 0.0001*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 8")
mesh.vdg(:,1,:) = 0.00007*tanh(nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 9")
mesh.vdg(:,1,:) = 0.00005*tanh(1.5*nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");
...
disp("Iter 10")
mesh.vdg(:,1,:) = 0.00004*tanh(1.5*nm*dist.*ld);
mesh.udg = sol;
preprocessing(pde,mesh);
runcode(pde, 1); % run C++ code
sol = fetchsolution(pde,master,dmd, pde.datapath + "/dataout");

...
function make_cylinder_mesh(porder)
    p, t = rectangular_quad_mesh(31, 21)
    f0 = zeros(Int, 4, size(t, 2))
    dgnodes = Exasim.Preprocessing.createdgnodes(p, t, f0, [], [], porder)

    p[1, :] .= logdec_values(p[1, :], 6.0)
    dgnodes[:, 1, :] .= logdec_values(dgnodes[:, 1, :], 6.0)
    p = map_halfcircle_points(p)
    dgnodes = map_halfcircle_dgnodes(dgnodes)

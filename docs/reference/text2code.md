...
The scaffold builds on **CPU or GPU**. CPU (`cpu`/`cpumpi` variant) is the default. Pass
`-DEXASIM_GPU=ON` to build the CUDA `gpu`/`gpumpi` variant: the driver selects the device
backend at compile time (`_CUDA` → backend 2) and `solve_steady` wraps device pointers

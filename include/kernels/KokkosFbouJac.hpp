...
static void KokkosFbouJacTemplate(dstype* f, dstype* f_udg,
                                  ...
                                  const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("FbouJac", ng, KOKKOS_LAMBDA(const size_t i) {
        constexpr int nd = Model::nd;
        constexpr int ncu = Model::ncu;
        constexpr int nc = ncu * (1 + nd);
        constexpr int nco = Model::nco;
        ...
        dstype fb_local[ncu];
        dstype fb_uq_local[ncu * nc];
        dstype fb_w_local[(ncw > 0) ? ncu * ncw : 1];
        dstype fb_uh_local[ncu * ncu];
        ...
        Model::fbou(fb_local, ib, x, uq, v, w, uh, n, tau_local, param,
                    uinf, time);
        Model::fbou_jac_uq(fb_uq_local, ib, x, uq, v, w, uh, n, tau_local,
                           param, uinf, time);
        Model::fbou_jac_w(fb_w_local, ib, x, uq, v, w, uh, n, tau_local,
                          param, uinf, time);
        Model::fbou_jac_uh(fb_uh_local, ib, x, uq, v, w, uh, n, tau_local,
                           param, uinf, time);

        for (int k = 0; k < ncu; ++k) f[k * ng + i] = fb_local[k];
        for (int k = 0; k < ncu * nc; ++k) f_udg[k * ng + i] = fb_uq_local[k];
        for (int k = 0; k < ncu * ncw; ++k) f_wdg[k * ng + i] = fb_w_local[k];
        for (int k = 0; k < ncu * ncu; ++k) f_uhg[k * ng + i] = fb_uh_local[k];
    });
}

void KokkosFbouJac(dstype* f, dstype* f_udg, dstype* f_wdg,
...
{
    KokkosFbouJacTemplate<PdeModel>(f, f_udg, f_wdg, f_uhg, xdg, udg, odg,

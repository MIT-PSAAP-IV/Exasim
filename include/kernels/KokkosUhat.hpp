...
static void KokkosUhatTemplate(dstype* f, const dstype* xdg,
                               ...
                               const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("Uhat", ng, KOKKOS_LAMBDA(const size_t i) {
        constexpr int nd = Model::nd;
        constexpr int ncu = Model::ncu;
        constexpr int nc = ncu * (1 + nd);
        constexpr int nco = Model::nco;
        ...
        Model::uhat(uhat_local, x, uq1, uq2, v1, v2, w1, w2, uh, n, tau_local,
                    param, uinf, time);

        for (int k = 0; k < ncu; ++k) f[k * ng + i] = uhat_local[k];
    });
}
...
{
    KokkosUhatTemplate<PdeModel>(f, xdg, udg1, udg2, odg1, odg2, wdg1, wdg2,

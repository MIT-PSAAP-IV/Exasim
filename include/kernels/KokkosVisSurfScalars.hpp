...
static void KokkosVisSurfScalarsTemplate(dstype* f, const dstype* xdg,
                                       ...
                                       const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("VisSurfScalars", ng, KOKKOS_LAMBDA(const size_t i) {
        constexpr int nd = Model::nd;
        constexpr int ncu = Model::ncu;
        constexpr int nc = ncu * (1 + nd);
        constexpr int nco = Model::nco;
        ...
        Model::vis_surf_scalars(out_local, ib, x, uq, v, w, uh, n, tau_local,
                                param, uinf, time);

        for (int k = 0; k < nsurfsca_runtime; ++k) f[k * ng + i] = out_local[k];
    });
}

void KokkosVisSurfScalars(dstype* f, const dstype* xdg, const dstype* udg, const dstype* odg, const dstype* wdg, const dstype* uhg, const dstype* nlg, const dstype* tau, const dstype* uinf, const dstype* param, const dstype time, const int modelnumber, const int ib, const int ng, const int nc, const int ncu, const int nd, const int ncx, const int nco, const int ncw)
{
    KokkosVisSurfScalarsTemplate<PdeModel>(f, xdg, udg, odg, wdg, uhg, nlg, tau,

// Pointwise surface quantities (heat flux, skin friction, Cp, ...) evaluated on the
// ibs boundaries: Model::surface_quantities has the qoi_boundary signature, but the
// result is written out per point (outbousurf_np*.bin) rather than integrated.
// `nsurfq_runtime` is the number of outputs (the ABI passes nsurfq in the nc slot).
// Only instantiated for models that define surface_quantities (see
// exasim_has_surface_quantities in driver_abi.hpp).
template <class Model>
static void KokkosSurfaceQuantitiesTemplate(dstype* f, const dstype* xdg,
                                            const dstype* udg, const dstype* odg,
                                            const dstype* wdg, const dstype* uhg,
                                            const dstype* nlg, const dstype* tau,
                                            const dstype* uinf, const dstype* param,
                                            const dstype time, const int modelnumber,
                                            const int ib, const int ng,
                                            const int nsurfq_runtime,
                                            const int ncu_runtime,
                                            const int nd_runtime, const int ncx,
                                            const int nco_runtime,
                                            const int ncw_runtime)
{
    (void)modelnumber;
    (void)ncu_runtime;
    (void)nd_runtime;
    (void)ncx;
    (void)nco_runtime;
    (void)ncw_runtime;

    Kokkos::parallel_for("SurfaceQuantities", ng, KOKKOS_LAMBDA(const size_t i) {
        constexpr int nd = Model::nd;
        constexpr int ncu = Model::ncu;
        constexpr int nc = ncu * (1 + nd);
        constexpr int nco = Model::nco;
        constexpr int ncw = Model::ncw;
        constexpr int ntau = Model::ntau;
        constexpr int kMax = 32;
        dstype x[nd];
        dstype uq[nc];
        dstype v[(nco > 0) ? nco : 1];
        dstype w[(ncw > 0) ? ncw : 1];
        dstype uh[ncu];
        dstype n[nd];
        dstype tau_local[ntau];
        dstype out_local[kMax];

        for (int k = 0; k < kMax; ++k) out_local[k] = 0.0;
        for (int k = 0; k < nd; ++k) x[k] = xdg[k * ng + i];
        for (int k = 0; k < nc; ++k) uq[k] = udg[k * ng + i];
        for (int k = 0; k < nco; ++k) v[k] = odg[k * ng + i];
        for (int k = 0; k < ncw; ++k) w[k] = wdg[k * ng + i];
        for (int k = 0; k < ncu; ++k) uh[k] = uhg[k * ng + i];
        for (int k = 0; k < ntau; ++k) tau_local[k] = tau[k];
        for (int k = 0; k < nd; ++k) n[k] = nlg[k * ng + i];

        Model::surface_quantities(out_local, ib, x, uq, v, w, uh, n, tau_local,
                                  param, uinf, time);

        const int nout = (nsurfq_runtime < kMax) ? nsurfq_runtime : kMax;
        for (int k = 0; k < nout; ++k) f[k * ng + i] = out_local[k];
    });
}

...
static void KokkosFhatTemplate(dstype* f, const dstype* xdg,
                               ...
                               const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("Fhat", ng, KOKKOS_LAMBDA(const size_t i) {
        ...
        Model::fhat(fh_local, x, uq1, uq2, v1, v2, w1, w2, uh, n, tau_local,
                    param, uinf, time);

        for (int k = 0; k < ncu; ++k) f[k * ng + i] = fh_local[k];
    });
}
...
{
    KokkosFhatTemplate<PdeModel>(f, xdg, udg1, udg2, odg1, odg2, wdg1, wdg2,

...
static void HdgFbouonlyTemplate(dstype* f, const dstype* xdg,
                                ...
                                const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("HdgFbouonly", ng, KOKKOS_LAMBDA(const size_t i) {
        constexpr int nd = Model::nd;
        constexpr int ncu = Model::ncu;
        constexpr int nc = ncu * (1 + nd);
        constexpr int nco = Model::nco;
...
{
    HdgFbouonlyTemplate<PdeModel>(f, xdg, udg, odg, wdg, uhg, nlg, tau, uinf,
